...
exports.onSocket = function (log, socket) {
    var getConnections = function (data, broadcast, callback) {
        ...
        SpeedDating.find({deck: data.deck, slide: data.slide, updated: {$gte: today}},'username',function (err, connections) {
                if (err) {
                    console.error(err);
                    //res([]);
                    return;
    ...
    socket.on('speedDating.nextRound', function (data) {
        if(data.round !== undefined && queueTable[data.round]){
            ...
            queueTable[data.round].forEach(function(el){
                if(el[0].username === socket.handshake.user.name) {
                   myQueue = {type:'call',user:el[1]};
                } else if (el[1].username === socket.handshake.user.name) {
                   myQueue = {type:'answer',user:el[0]};
                }
            });
